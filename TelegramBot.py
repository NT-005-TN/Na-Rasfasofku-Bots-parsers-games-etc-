...
information_about_deliver = [['Нет данных'],['Нет данных'],['Нет данных'],['Нет данных']]
...
def callback_worker(call):
	...
	if call.data == 'deliver':
		...
		information_about_deliver[2] = 'Нет'
		add_information_about_deliver(call.message)
...
def get_deliver_phone_number(message):
	...
	if not fnmatch(phone_number, '+7??????????'):
		...
		bot.register_next_step_handler(message, get_deliver_name)
	else:
